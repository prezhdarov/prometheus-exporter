...
The files in **example** folder are the absolute minimum necessary to build a working exporter.
...
This is where the target consumption takes place. The client (APIClient) requires three methods - a login, a get and a logoout functions - to authenticate, read and clean any loose ends for every scrape. Again follow the comments in file to create your own.

### A set of collectors (example has only one in collcectors/example.go)

This is a set of metrics collectors sharing a package name. Each collector executes concurently and must have unique name and needs an update method which will be called by the Collect function.
